...
type
  ...
  public
    ...
    // added by TS
    class function ABGenerateValidRandomBytesForPrivateKey(): TBytes; static;

    //added by TS


    class procedure ABGetPublicKeyDetails(const APrivateKey: TBytes;
   var Address, PubKey, AddressComp: String; incAdr, incAdrComp: Boolean); static;



    class procedure GetPublicKeyDetails(const APrivateKey: String;
       var Address, PubKey, AddressComp: String); static;

    class procedure GetPublicKeyDetailsKPE(const APrivateKey: String;
       var Address, PubKey, PubKeyComp, AddressComp, Hash160, Hash160Comp: String);  static;
...
class function TbtcKeyFunctions.ABGenerateValidRandomBytesForPrivateKey(): TBytes;
begin
  Result := TBitcoinKey.GenerateValidRandomBytesForPrivateKey();
end;

class procedure TbtcKeyFunctions.ABGetPublicKeyDetails(const APrivateKey: TBytes;
   var Address, PubKey, AddressComp: String; incAdr, incAdrComp: Boolean);
var
  Key: IBitcoinKey;
begin
  Key := TBitcoinKey.Create(APrivateKey, -1, false);

  if incAdr then Address := Key.GeneratePublicAddress(false);
  if incAdrComp then AddressComp := Key.GeneratePublicAddress(true);


  //PubKey := THex.Encode(Key.PublicKey.Q.Normalize.GetEncoded(false));
...
class procedure TbtcKeyFunctions.GetPublicKeyDetails(const APrivateKey: String;
   var Address, PubKey, AddressComp: String);
var
  Key: IBitcoinKey;
begin
  Key := TBitcoinKey.Create(THex.Decode(APrivateKey), -1, false);
  Address := Key.GeneratePublicAddress(false);
  AddressComp := Key.GeneratePublicAddress(true);
  //PubKey := THex.Encode(Key.PublicKey.Q.Normalize.GetEncoded(false));
...
class procedure TbtcKeyFunctions.GetPublicKeyDetailsKPE(const APrivateKey: String;
   var Address, PubKey, PubKeyComp, AddressComp: String);
var
  Key: IBitcoinKey;
begin
  Key := TBitcoinKey.Create(THex.Decode(APrivateKey), -1, false);
  ...
  //Key := TBitcoinKey.Create(THex.Decode(APrivateKey), -1, true);

  PubKeyComp := THex.Encode(Key.PublicKey.Q.Normalize.GetEncoded(true));

end;

class procedure TbtcKeyFunctions.GetPublicKeyDetailsKPE(const APrivateKey: String;
   var Address, PubKey, PubKeyComp, AddressComp, Hash160, Hash160Comp: String);
var
  Key: IBitcoinKey;
begin
  Key := TBitcoinKey.Create(THex.Decode(APrivateKey), -1, false);
  Hash160:=key.GeneratePublicAddress160(false);
  Hash160Comp:=key.GeneratePublicAddress160(true);
  Address := Key.GeneratePublicAddress(false);
  AddressComp := Key.GeneratePublicAddress(true);
  PubKey := THex.Encode(Key.PublicKey.Q.Normalize.GetEncoded(false));
